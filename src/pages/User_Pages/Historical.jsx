...
/* ---------------------------------------------------------
   Thematic images — reuse the same photo-grid trick as the
   NepalFlow landing page so this page doesn't feel like a
   plain data dashboard. Swap for your own /public assets
   (e.g. `${import.meta.env.BASE_URL}bouddha.png`) any time.
--------------------------------------------------------- */
const HERO_IMG =
  "https://images.unsplash.com/photo-1585511543399-cb3c6395e6e1?w=1600&q=80";
const SECTION_IMAGES = {
  trend:
    "https://images.unsplash.com/photo-1516912481808-3406841bd33c?w=800&q=80", // Annapurna range
  purpose:
    "https://images.unsplash.com/photo-1605640840605-14ac1855827b?w=800&q=80", // Everest
  mode:
    "https://images.unsplash.com/photo-1533130061792-64b345e4a833?w=800&q=80", // Boudhanath
  season:
    "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80", // Phewa Lake
};

const style = {
  card:
    "bg-background text-foreground border border-gray-500/40 rounded-2xl p-6 hover:border-orange-400/40 transition-all duration-300",
  hero:
    "relative overflow-hidden rounded-3xl border border-gray-500/40",
  sectionTitle: "font-semibold flex items-center gap-2 text-lg",
  subtext: "text-sm text-gray-400 mt-1 mb-2 max-w-2xl leading-relaxed",
  insight:
...
// ---- stat card, now with a subtle gradient glow instead of a sparkline slot ----
function StatCard({ label, value, decimals = 2, suffix = "", sub, subColor, icon }) {
  const animated = useCountUp(value);
  return (
    <div className={`${style.card} relative overflow-hidden group`}>
      <div className="absolute -right-4 -top-4 text-6xl opacity-[0.06] group-hover:opacity-[0.12] transition-opacity">
        {icon}
      </div>
      <div className="relative">
        <div className={style.statLabel}>{label}</div>
        <div className="text-2xl md:text-3xl font-bold text-orange-400 tabular-nums">
          {animated.toFixed(decimals)}
          {suffix}
        </div>
        {sub && (
          <div className="text-xs mt-1 font-medium" style={{ color: subColor || "#9ca3af" }}>
            {sub}
          </div>
        )}
      </div>
...
// ---- small reusable "section header with a photo chip" ----
function SectionHeader({ icon, title, subtitle, img }) {
  return (
    <div className="flex items-start justify-between gap-4 mb-1">
      <div>
        ...
        {subtitle && <p className={style.subtext}>{subtitle}</p>}
      </div>
      {img && (
        <img
          src={img}
          alt=""
          className="hidden sm:block w-16 h-16 rounded-xl object-cover border border-gray-500/30 shrink-0"
        />
      )}
    </div>
  );
  ...
  return (
    <div className="bg-background text-foreground min-h-screen">
      <div className="max-w-6xl mx-auto p-6 space-y-10">
        ...
        {/* ---------------- Hero, image-backed like the landing page ---------------- */}
        <div className={style.hero}>
          <img
            src={HERO_IMG}
          ...
          <div className="relative z-10 p-8 md:p-12">
            <div className="flex items-center gap-2 text-xs text-orange-300 uppercase tracking-widest mb-3">
              <FaMountain />
              <span>Nepal Tourism Intelligence</span>
            </div>
            ...
            <p className="text-gray-200 text-sm max-w-xl mb-8 leading-relaxed">
              An analytical deep dive into the resilient journey of the Himalayan
              nation's primary economic engine, spanning through triumph and recovery.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatCard
                label={`${stats.low.year} Historic Low`} value={stats.low.total / 1000} decimals={0} suffix="K"
                sub="Global Lockdown" subColor="#f0a894" icon={<FaPlaneArrival />}
              />
              <StatCard
                label={`${stats.current.year} Current`} value={stats.current.total / 1e6} suffix="M"
                sub={`+${stats.current.percentChange}% Recovery`} subColor="#8fd19e" icon={<FaUsers />}
              />
              <StatCard
                label="Historical" value={stats.cagr || 0} suffix="%" sub="15-Year Avg" icon={<FaCalendarAlt />}
              />
            </div>
          </div>
        </div>
        ...
        <div className={style.card}>
          <SectionHeader
            ...
            title={`Annual Arrival Trend ${arrivalsByYear[0].year}–${arrivalsByYear[arrivalsByYear.length - 1].year}`}
            subtitle="Global visitor volume with major geopolitical and natural event markers overlaid."
            img={SECTION_IMAGES.trend}
          />
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className={style.card}>
            <SectionHeader
              title="Purpose of Visit, Over Time"
              subtitle="How travel motivations have shifted year over year."
              img={SECTION_IMAGES.purpose}
            />
            <div style={{ height: 260 }} className="mt-4">
              <Line data={purposeData} options={purposeOptions} />
            </div>
            <p className={style.insight}>{insights.purpose}</p>
          </div>
          <div className={style.card}>
            <SectionHeader
              icon={<FaPlaneArrival className="text-orange-400 text-base" />}
              title="Entry Mode: Air vs Land"
              subtitle="Share of arrivals by air vs overland border crossing."
              img={SECTION_IMAGES.mode}
            />
            <div style={{ height: 260 }} className="mt-4">
              <Bar data={modeData} options={modeOptions} />
            </div>
            <p className={style.insight}>{insights.mode}</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className={`${style.card} md:col-span-2`}>
            <SectionHeader
              ...
              subtitle={`Monthly arrival intensity, ${heatmapYears[0].year}–${heatmapYears[heatmapYears.length - 1].year}. Darker = higher volume.`}
              img={SECTION_IMAGES.season}
            />
                      ...
                      {months.map((m) => {
                        ...
                        return (
                          <td key={m} className="p-0">
                            <div
                              title={`${m} ${row.year}: ${v.toLocaleString()}`}
                              className="w-full h-5 rounded-sm hover:scale-y-125 transition-transform"
                              style={{ backgroundColor: `rgba(251,146,60,${0.08 + intensity * 0.85})` }}
                            />
                          </td>
                        );
